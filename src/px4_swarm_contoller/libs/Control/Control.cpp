...
ctrl::Control::Control() : Node("Control"){
    ...
    // std::cout << "Number of setpoints for each drone: " << num_setpoints_ << std::endl;
    ...
    for(int i=1; i<=num_drones_; i++){
        ...
        std::string local_position_topic = "/px4_" + std::to_string(i) + "/fmu/out/vehicle_local_position";
    ...
    auto timer_callback = [this]() -> void{
        ...
        // After enough setpoints and offboard control mode messages have been sent, arm the drones and switch to offboard mode
        ...
        for(int i=0; i<num_drones_; i++){
            ...
            // After arming and switching to offboard mode, publish the setpoints read from the yaml file
...
void ctrl::Control::publish_default_setpoints(){
    ...
    for(int i=0; i<num_drones_; i++){
        ...
        publish_trajectory_setpoint(trajectory_setpoint_publishers_[i], setpoint[0], setpoint[1], setpoint[2], setpoint[3]);

        // Calculate the distance of the drone from its setpoint
        float distance = std::sqrt(std::pow(local_positions_[i][0] - setpoint[0], 2) + 
                         std::pow(local_positions_[i][1] - setpoint[1], 2) +
                         std::pow(local_positions_[i][2] - setpoint[2], 2));

        // If the drone is close to its setpoint and has not reached its setpoint before, set the reached_setpoint_ flag to true
        if(distance < 0.3 and !reached_setpoint_[i]) {
            reached_setpoint_[i] = true;
        }

        // If all the drones have reached their setpoints, increment the setpoint_counter_, but stop it after reaching the last setpoint
        if(std::all_of(reached_setpoint_.begin(), reached_setpoint_.end(), [](bool v) { return v; })) {
            setpoint_counter_++;
        ...
        //     // std::cout << "Drone " << i+1 << " current position: x=" << local_positions_[i][0] << ", y=" << local_positions_[i][1] << ", z=" << local_positions_[i][2] << std::endl;
        //     // std::cout << "Drone " << i+1 << " setpoint: x=" << setpoint[0] << ", y=" << setpoint[1] << ", z=" << setpoint[2] << std::endl;

        //     // Print the distance from the setpoint
        //     std::cout << "Drone " << i+1 << " distance from setpoint: " << distance << std::endl;
        // }
    }
}

void ctrl::Control::vehicle_local_position_callback(int drone_id, const VehicleLocalPosition::SharedPtr msg){
    local_positions_[drone_id] = {msg->x, msg->y, msg->z};
    // std::cout << "Drone " << drone_id << " local position: x=" << msg->x << ", y=" << msg->y << ", z=" << msg->z << std::endl;
}